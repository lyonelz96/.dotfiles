...
if comment_api_ok then
	...
	nmap('<leader>cl', comment_api.toggle.linewise.current, { desc = '[C]omment Current [L]ine' })

	nmap('<leader>cc', function()
		comment_api.toggle.linewise.count(vim.v.count)
	end, { desc = '[C]omment [C]ount' })

	nmap('<leader>co', comment_api.call('toggle.linewise', 'g@'), { desc = '[C]omment [O]perator', expr = true })
